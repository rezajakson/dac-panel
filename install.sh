...
echo -e "${PURPLE}====================================================${NC}"
echo -e "${PURPLE}       DAC Panel - Auth Separated Version           ${NC}"
echo -e "${PURPLE}====================================================${NC}"

# رفع خودکار خطای اشغال بودن پورت 3000
echo -e "${CYAN}[Pre-flight] Killing old processes on port 3000...${NC}"
...
echo -e "${CYAN}[1/4] Downloading Fonts...${NC}"
...
{
  "name": "dac-panel-server", "version": "3.0.0", "type": "module",
...
app.use(session({ secret: 'dac_super_secret', resave: false, saveUninitialized: true, cookie: { secure: false } }));
...
db.exec(`
  ...
  /* ایجاد کاربر پیش‌فرض ادمین اگر وجود نداشت */
...
// ==========================================
// سیستم احراز هویت خود پنل (جدا از X-UI)
// ==========================================
...
app.post('/api/auth/change-password', (req, res) => {
    ...
    const user = db.prepare('SELECT * FROM panel_users WHERE id = ? AND password = ?').get(req.session.userId, oldPassword);
    if (user) {
        db.prepare('UPDATE panel_users SET password = ? WHERE id = ?').run(newPassword, req.session.userId);
        res.json({ success: true, message: 'رمز عبور با موفقیت تغییر کرد' });
    } else {
        res.status(400).json({ error: 'رمز عبور فعلی اشتباه است' });
    }
});

// ==========================================
// سیستم تنظیمات اتصال به X-UI
// ==========================================
app.get('/api/settings/xui', (req, res) => {
    if (!req.session.loggedIn) return res.status(401).json({ error: "Not logged in" });
    const row = db.prepare('SELECT value FROM settings WHERE key = ?').get('xui_conn');
    if (row) { const s = JSON.parse(row.value); s.password = dec(s.password); res.json(s); } 
    else res.json({ url: '', username: '', password: '' });
});
...
app.post('/api/settings/xui', (req, res) => {
    ...
    res.json({ success: true, message: 'اتصال به پنل ذخیره شد' });
});
...
async function getXui() {
    const row = db.prepare('SELECT value FROM settings WHERE key = ?').get('xui_conn');
    if (!row) throw new Error("لطفا ابتدا اتصال به پنل را در تنظیمات برقرار کنید");
...
app.listen(PORT, '0.0.0.0', () => console.log(`\n🚀 DAC Panel v3.0 running on port ${PORT}\n`));
SERVERCODE
...
{
  "name": "dac-ui", "private": true, "version": "3.0.0", "type": "module",
...
# فایل‌های تنظیمات ری‌اکت (بدون تغییر)
...
# ==========================================
# فرانت‌اند (اضافه شدن صفحه لاگین مجزا و تنظیمات کامل)
# ==========================================
...
import { LayoutDashboard, Users, UserPlus, UserCog, Settings, LogOut, Menu, X, Plus, RefreshCw, Ban, Trash2, Wifi, Save, Shield } from 'lucide-react';

const api = axios.create({ baseURL: '/api', withCredentials: true });

export default function App() {
    const [loggedIn, setLoggedIn] = useState(false);
    const [auth, setAuth] = useState({ username: '', password: '' }); // فقط یوزر و پس پنل
    ...
    const login = async (e) => { 
        e.preventDefault(); 
        try { 
            const r = await api.post('/auth/login', auth); 
            if(r.data.success) { setLoggedIn(true); fetchAll(); } 
            else alert(r.data.message);
        } catch(e) { alert("خطا در اتصال"); } 
    };
    
    const fetchAll = () => { 
        api.get('/data').then(r => setData(r.data)).catch(()=>{}); 
        api.get('/agents').then(r => setAgents(r.data)); 
        api.get('/settings/xui').then(r => setXuiSettings(r.data)); 
    };

    const mkUser = async () => { await api.post('/users/create', uForm); setModal({open:false, type:''}); fetchAll(); };
    const uAct = async (a, v) => { await api.post('/users/action', { userId: drawer.user.id, inboundId: drawer.user.inboundId, action: a, value: v }); setDrawer({open:false, user:null}); fetchAll(); };
    const mkAgent = async () => { await api.post('/agents', aForm); setModal({open:false, type:''}); fetchAll(); };

    const saveXui = async () => {
        try {
            const r = await api.post('/settings/xui', xuiSettings);
            setMsg(r.data.message);
            setTimeout(()=>setMsg(''), 3000);
            fetchAll();
        } catch(e) { setMsg('خطا در ذخیره'); }
    };

    const changePass = async () => {
        try {
            const r = await api.post('/auth/change-password', passForm);
            setMsg(r.data.message);
            setPassForm({oldPassword:'', newPassword:''});
            setTimeout(()=>setMsg(''), 3000);
        } catch(e) { setMsg(e.response?.data?.error || 'خطا'); }
    };
    ...
    if (!loggedIn) return (
        <div className="min-h-screen bg-animated flex items-center justify-center p-4">
            <motion.form onSubmit={login} initial={{opacity:0, y:50}} animate={{opacity:1,y:0}} className="w-full max-w-md bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl">
                ...
                <button type="submit" className="w-full py-3 rounded-xl bg-gradient-to-l from-purple-600 to-blue-600 text-white font-bold hover:opacity-90 transition flex items-center justify-center gap-2"><Shield size={18}/> ورود به پنل</button>
            </motion.form>
        </div>
    );
            ...
            <main className="flex-1 relative z-10 p-6 md:p-10 overflow-y-auto">
                ...
                {page === 'agents' && (
                    <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="space-y-4">
                        ...
                        <div className="grid md:grid-cols-2 gap-4">{agents.map(a=>(<div key={a.id} className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-6"><h3 className="font-bold text-lg text-purple-400">{a.name}</h3><p className="text-gray-400 text-sm">@{a.username} | {a.phone}</p><p className="mt-4">موجودی: <span className="text-green-400 font-bold">{a.balance} تومان</span></p><button onClick={()=>{const v=prompt('مبلغ شارژ:'); if(v) api.post('/agents/charge',{id:a.id,amount:v}).then(fetchAll);}} className="text-xs mt-4 px-3 py-1 bg-yellow-500/20 text-yellow-300 rounded-lg">شارژ</button></div>))}</div>
                    </motion.div>
                )}
                    ...
                    <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="max-w-3xl mx-auto space-y-8">
                        ...
                        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
                            ...
                            <div className="space-y-4">
                                <input placeholder="آدرس پنل (مثل http://1.2.3.4:2053)" value={xuiSettings.url} onChange={e=>setXuiSettings({...xuiSettings, url:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white focus:outline-none focus:border-purple-500"/>
                                <div className="grid grid-cols-2 gap-4">
                                    <input placeholder="نام کاربری x-ui" value={xuiSettings.username} onChange={e=>setXuiSettings({...xuiSettings, username:e.target.value})} className="bg-white/5 border border-white/10 rounded-lg p-3 text-white focus:outline-none focus:border-purple-500"/>
                                    <input type="password" placeholder="رمز عبور x-ui" value={xuiSettings.password} onChange={e=>setXuiSettings({...xuiSettings, password:e.target.value})} className="bg-white/5 border border-white/10 rounded-lg p-3 text-white focus:outline-none focus:border-purple-500"/>
                                </div>
                                <motion.button whileTap={{scale:0.95}} onClick={saveXui} className="w-full py-3 rounded-xl bg-purple-600/30 text-purple-200 border border-purple-500/30 hover:bg-purple-600/50 transition flex items-center justify-center gap-2"><Save size={18}/> ذخیره اتصال</motion.button>
                            </div>
                        </div>

                        {/* کارد تغییر رمز پنل DAC */}
                        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
                            <h3 className="text-lg font-bold text-blue-400 mb-4">تغییر رمز عبور پنل DAC</h3>
                            <div className="space-y-4">
                                <input type="password" placeholder="رمز عبور فعلی" value={passForm.oldPassword} onChange={e=>setPassForm({...passForm, oldPassword:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white focus:outline-none focus:border-blue-500"/>
                                <input type="password" placeholder="رمز عبور جدید" value={passForm.newPassword} onChange={e=>setPassForm({...passForm, newPassword:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white focus:outline-none focus:border-blue-500"/>
                                <motion.button whileTap={{scale:0.95}} onClick={changePass} className="w-full py-3 rounded-xl bg-blue-600/30 text-blue-200 border border-blue-500/30 hover:bg-blue-600/50 transition flex items-center justify-center gap-2"><Shield size={18}/> تغییر رمز عبور</motion.button>
                            </div>
                        </div>
                    </motion.div>
                )}
...
if [ ! -f "dist/index.html" ]; then
    clear
    echo -e "${RED}❌ Error during build process.${NC}"
    cat build.log
    exit 1
fi
...
clear
echo -e "${GREEN}====================================================${NC}"
echo -e "${GREEN}             ✅ DAC Panel v3.0 Ready!                ${NC}"
echo -e "${GREEN}====================================================${NC}"
echo -e "${NC}Default Login: ${PURPLE}admin${NC} / ${PURPLE}admin${NC}"
echo -e "Run persistently: ${PURPLE}npm i -g pm2 && pm2 start /root/dac-panel/server/index.js --name dac${NC}"
