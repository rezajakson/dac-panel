...
echo -e "${PURPLE}====================================================${NC}"
echo -e "${PURPLE}       DAC Panel - Stable Version Installer          ${NC}"
echo -e "${PURPLE}====================================================${NC}"
...
# ==========================================
# 1. دانلود فونت‌ها از گیت‌هاب (بسیار حرفه‌ای)
# ==========================================
echo -e "${CYAN}[1/4] Downloading Local Fonts...${NC}"
# ⚠️ توجه: در گیت‌هاب لینک raw فایل‌ها رو جایگزین کنید
FONT_BASE_URL="https://raw.githubusercontent.com/YOUR_USERNAME/DAC-Panel/main"
curl -L -o client/public/fonts/Vazirmatn-Regular.woff2 "$FONT_BASE_URL/Vazirmatn-Regular.woff2" 2>/dev/null || echo "Warning: Failed to download Regular font (using fallback)"
curl -L -o client/public/fonts/Vazirmatn-Bold.woff2 "$FONT_BASE_URL/Vazirmatn-Bold.woff2" 2>/dev/null || echo "Warning: Failed to download Bold font (using fallback)"
...
app.use(session({ secret: 'dac_panel_secret_key_v2', resave: false, saveUninitialized: true, cookie: { secure: false } }));
    ...
    try {
        ...
        if (resXui.data.success) {
            db.prepare(`INSERT INTO settings (key, value) VALUES ('xui_conn', ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value`)
                .run(JSON.stringify({ url, username, password: enc(password) }));
        ...
        inbounds.forEach(ib => {
            (ib.clientStats || []).forEach(cs => {
                ...
                if (client) {
                    ...
                    const agent = mapping ? db.prepare('SELECT name FROM agents WHERE id = ?').get(mapping.agent_id) : null;
                    let status = 'active';
                    ...
                    allUsers.push({ id: client.id, inboundId: ib.id, inboundTag: ib.tag, protocol: ib.protocol, name: client.id, enable: client.enable, expiryTime: client.expiryTime, totalGB: client.totalGB, up: cs.up, down: cs.down, status, agentName: agent ? agent.name : 'مدیر سیستم', subLink: client.subId ? `${xui.baseURL}/sub/${client.subId}` : null });
                }
            });
        });
...
app.post('/api/users/create', async (req, res) => {
    try {
        ...
        const newUser = { id: name || `dac_${uuidv4().split('-')[0]}`, email: `${name || uuidv4().split('-')[0]}@dac.ir`, enable: true, expiryTime, totalGB: gbLimit, subId: uuidv4() };
...
app.post('/api/users/action', async (req, res) => {
    try {
        ...
        if (action === 'addVolume') ib.clients[clientIdx].totalGB += parseFloat(value);
        else if (action === 'renew') { let base = ib.clients[clientIdx].expiryTime > Date.now()/1000 ? ib.clients[clientIdx].expiryTime : Math.floor(Date.now()/1000); ib.clients[clientIdx].expiryTime = base + (parseInt(value) * 86400); ib.clients[clientIdx].enable = true; }
        else if (action === 'toggle') ib.clients[clientIdx].enable = !ib.clients[clientIdx].enable;
        else if (action === 'delete') ib.clients.splice(clientIdx, 1);
        await axios.post(`${xui.baseURL}/panel/inbound/update/${inboundId}`, ib, { headers: xui.headers });
        res.json({ success: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/agents', (req, res) => { try { const { username, password, name, phone, default_inbound_id } = req.body; db.prepare('INSERT INTO agents (username, password, name, phone, default_inbound_id) VALUES (?, ?, ?, ?, ?)').run(username, password, name, phone, default_inbound_id); res.json({ success: true }); } catch(e) { res.status(400).json({error: e.message}); } });
...
# 3. فرانت‌اند
# ==========================================
echo -e "${CYAN}[3/4] Installing Frontend Libraries...${NC}"

cat << 'EOF' > client/package.json
{
  "name": "dac-ui", "private": true, "version": "2.1.0", "type": "module",
...
import { LayoutDashboard, Users, UserPlus, Handshake, Settings, LogOut, Menu, X, Plus, RefreshCw, Ban, Trash2, Wifi } from 'lucide-react';
...
export default function App() {
    ...
    const login = async (e) => { e.preventDefault(); try { const r = await api.post('/auth/login', authForm); if(r.data.success) { setLoggedIn(true); fetchAll(); } } catch(e) { alert("خطا در اتصال"); } };
    const fetchAll = () => { api.get('/data').then(r => setData(r.data)); api.get('/agents').then(r => setAgents(r.data)); };
    const handleCreateUser = async () => { await api.post('/users/create', userForm); setModal({open:false, type:''}); fetchAll(); };
    const handleUserAction = async (action, value) => { await api.post('/users/action', { userId: drawer.user.id, inboundId: drawer.user.inboundId, action, value }); setDrawer({open:false, user:null}); fetchAll(); };
    const handleCreateAgent = async () => { await api.post('/agents', agentForm); setModal({open:false, type:''}); fetchAll(); };

    const StatusLight = ({ status }) => { const c = { active: 'bg-green-400 pulse-green', disabled: 'bg-blue-900', expired: 'bg-red-500' }; return <div className={`w-3 h-3 rounded-full ${c[status] || 'bg-gray-500'}`}></div>; };
    const ProgressBar = ({ value, max }) => { const p = max === 0 ? 100 : Math.min((value / max) * 100, 100); const c = p > 50 ? 'from-emerald-500 to-green-400' : p > 20 ? 'from-yellow-500 to-orange-400' : 'from-red-500 to-rose-400'; return <div className="w-24 h-1.5 bg-gray-700/50 rounded-full overflow-hidden"><motion.div initial={{width:0}} animate={{width:`${p}%`}} transition={{duration:1}} className={`h-full bg-gradient-to-l ${c} rounded-full`}/></div>; };
    ...
    return (
        <div className="flex min-h-screen font-vazir text-white overflow-hidden relative">
            ...
            <motion.aside initial={{x:300}} animate={{x: sidebarOpen ? 0 : (window.innerWidth > 768 ? 0 : 300)}} className="fixed md:relative z-30 w-72 h-screen p-4 flex flex-col gap-2 bg-white/5 backdrop-blur-xl border-l border-white/10">
                <div className="flex items-center justify-between p-4 border-b border-white/10 mb-4"><h1 className="text-xl font-bold text-purple-400">DAC Panel</h1><button className="md:hidden" onClick={()=>setSidebarOpen(false)}><X size={24}/></button></div>
                <nav className="flex flex-col gap-1 flex-1">
                    {[{id:'dashboard', l:'داشبورد', i:<LayoutDashboard size={20}/>},{id:'users', l:'کاربران', i:<Users size={20}/>},{id:'agents', l:'نمایندگان', i:<Handshake size={20}/>},{id:'settings', l:'تنظیمات', i:<Settings size={20}/>}].map(m=>(
                        <motion.div key={m.id} whileHover={{x:-5}} onClick={()=>{setPage(m.id); setSidebarOpen(false);}} className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer text-gray-300 hover:bg-white/10 hover:text-white transition-all border-r-2 ${page===m.id?'bg-white/10 border-purple-500':'border-transparent'}`}>{m.i}<span className="text-sm">{m.l}</span></motion.div>
                    ))}
                </nav>
                ...
                {page === 'users' && (
                    <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="space-y-4">
                        ...
                        <div className="space-y-3">
                            {data.users.map(u=>{ const daysLeft = u.expiryTime > 0 ? Math.ceil((u.expiryTime*1000 - Date.now())/(86400000)) : '∞'; const usedGb = (u.up + u.down) / 1073741824; const totalGb = u.totalGB === 0 ? '∞' : u.totalGB; return (
                        ...
                        <div className="grid md:grid-cols-2 gap-4">
                            {agents.map(a=>(
                                <div key={a.id} className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-6">
                                    ...
                                    <button onClick={()=>{const v=prompt('مبلغ شارژ به تومان:'); if(v) api.post('/agents/charge',{id:a.id,amount:v}).then(fetchAll);}} className="text-xs mt-4 px-3 py-1 bg-yellow-500/20 text-yellow-300 rounded-lg">شارژ حساب</button>
                                </div>
                            ))}
                        </div>
                    </motion.div>
                ...
                {drawer.open && drawer.user && (
                    <>
                        ...
                        <motion.div initial={{x:500}} animate={{x:0}} exit={{x:500}} transition={{type:"spring", damping:25}} className="fixed left-0 top-0 h-full w-full max-w-md bg-slate-900/90 backdrop-blur-xl border-l border-white/10 z-50 p-6 shadow-2xl overflow-y-auto">
                            ...
                            <p className="text-sm text-gray-400 mb-6 break-all">ساب: <span className="text-blue-400">{drawer.user.subLink || 'ندارد'}</span></p>
                            {drawer.user.subLink && (<div className="flex justify-center mb-6 bg-white/5 p-4 rounded-xl"><QRCodeSVG value={drawer.user.subLink} size={180} bgColor="transparent" fgColor="#ffffff"/></div>)}
                            <div className="space-y-3">
                        ...
                        <motion.div initial={{scale:0.9, opacity:0}} animate={{scale:1, opacity:1}} exit={{scale:0.9, opacity:0}} className="fixed inset-0 m-auto w-fit h-fit bg-slate-900/90 backdrop-blur-xl border border-white/10 rounded-2xl p-8 z-50 shadow-2xl">
                            {modal.type === 'createUser' && (
                                <div className="w-96 space-y-4">
                                    ...
                                    <select value={userForm.inboundId} onChange={e=>setUserForm({...userForm, inboundId:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white">{data.inbounds.map(ib=><option key={ib.id} value={ib.id} className="bg-slate-800">{ib.tag} ({ib.protocol})</option>)}</select>
                                    <div className="grid grid-cols-2 gap-4"><input type="number" placeholder="روز اشتراک" value={userForm.daysLimit} onChange={e=>setUserForm({...userForm, daysLimit:e.target.value})} className="bg-white/5 border border-white/10 rounded-lg p-3 text-white"/><input type="number" placeholder="حجم (GB)" value={userForm.gbLimit} onChange={e=>setUserForm({...userForm, gbLimit:e.target.value})} className="bg-white/5 border border-white/10 rounded-lg p-3 text-white"/></div>
                                    ...
                                    <select value={userForm.agentId} onChange={e=>setUserForm({...userForm, agentId:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white"><option value="" className="bg-slate-800">بدون نماینده (مدیر)</option>{agents.map(a=><option key={a.id} value={a.id} className="bg-slate-800">{a.name}</option>)}</select>
                            ...
                            {modal.type === 'createAgent' && (
                                <div className="w-96 space-y-4">
                                    ...
                                    <select value={agentForm.default_inbound_id} onChange={e=>setAgentForm({...agentForm, default_inbound_id:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white"><option value="" className="bg-slate-800">اینباند پیش‌فرض انتخاب نشده</option>{data.inbounds.map(ib=><option key={ib.id} value={ib.id} className="bg-slate-800">{ib.tag}</option>)}</select>
...
# ==========================================
# 4. بیلد نهایی با رفع خطای کمبود رم
# ==========================================
echo -e "${CYAN}[4/4] Building Production Files (Allocating RAM)...${NC}"
cd client
npm install > /dev/null 2>&1

# افزایش حافظه مجاز نود برای جلوگیری از خطای ENOENT
export NODE_OPTIONS="--max-old-space-size=1024"
npm run build 2>&1 | tee build.log

# بررسی اینکه آیا بیلد با موفقیت انجام شد یا خیر
if [ ! -f "dist/index.html" ]; then
    clear
    echo -e "${RED}====================================================${NC}"
    echo -e "${RED}       ❌ خطا در ساخت فایل‌های فرانت‌اند          ${NC}"
    echo -e "${RED}====================================================${NC}"
    echo -e "${NC}"
    echo -e "سرور شما رم (RAM) بسیار کم دارد و نتوانست فایل‌ها را بیلد کند."
    echo -e "لاگ خطاها:"
    cat build.log
    exit 1
fi
...
clear
echo -e "${GREEN}====================================================${NC}"
echo -e "${GREEN}             ✅ DAC Panel v2.1 Installed!            ${NC}"
echo -e "${GREEN}====================================================${NC}"
echo -e "${NC}پنل با موفقیت نصب شد. برای اجرای دائمی دستورات زیر را بزنید:${NC}"
echo -e "${PURPLE}npm i -g pm2${NC}"
echo -e "${PURPLE}pm2 start /root/dac-panel/server/index.js --name dac${NC}"
echo -e "${CYAN}آدرس: http://YOUR_SERVER_IP:3000${NC}"
